...
func (s *server) ClearRoom(ctx context.Context, room *v1.RoomUser) (*v1.Empty, error) {
	if err := s.manager.ClearRoom(room.Id, room.UserId); err != nil {
		return &v1.Empty{}, fmt.Errorf("error occurred when clearing room: %s, %w", room.Id, err)
	}
	log.Println("ClearRoom() => room:", room.Id, "was cleared by:", room.UserId)
	return &v1.Empty{}, nil
}
