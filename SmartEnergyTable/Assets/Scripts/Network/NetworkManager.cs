...
namespace Network
{
    ...
    public sealed class NetworkManager : MonoBehaviour
    {
        ...
        private ViewMode _currentViewMode = ViewMode.Overview;
        ...
        public readonly Dictionary<string, GameObject> _currentScene = new Dictionary<string, GameObject>();
        ...
        private readonly Dictionary<string, Action<ViewMode>> _viewModeListeners =
            new Dictionary<string, Action<ViewMode>>();
        ...
        /// <param name="uuid">This is an identifier for the listener</param>
        public void ObserveViewMode(string uuid, Action<ViewMode> callback)
        {
            _viewModeListeners.Add(uuid, callback);
            foreach (var viewModeListener in _viewModeListeners)
            {
                viewModeListener.Value.Invoke(_currentViewMode);
            }
        }
        ...
        /// <summary>
        /// UnObserveViewMode: When a listener no longer needs to listen they should unsubscribe.
        /// </summary>
        /// <param name="uuid">This is an identifier for the listener</param>
        public void UnObserveViewMode(string uuid)
        {
            _viewModeListeners.Remove(uuid);
        }
                ...
                await _client.JoinRoom(id, _userId, patch =>
                {
                    lock (_actionQueue)
                    {
                        ...
                        _actionQueue.Enqueue(() =>
                        {
                            ...
                            if (patch.SceneId != SceneManager.GetActiveScene().buildIndex && !_firstRun)
                            {
        ...
        /// <summary>
        /// Switch between Overview and StreetView mode.
        /// </summary>
        /// <param name="mode">This can be either "overview" or "streetview"</param>
        public void SwitchViewMode(string mode)
        {
            ViewMode m;
            if (mode.ToLower() == "overview")
                m = ViewMode.Overview;
            else if (mode.ToLower() == "streetview")
                m = ViewMode.StreetView;
            else
            {
                Debug.Log(mode + " is not supported.");
                return;
            }
